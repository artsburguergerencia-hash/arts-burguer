...
async def webhook_mercadopago(request:Request, db: Session = Depends(get_db)): try: payload = await request.json() return {"status": "ok"} except Exception: return {"status": "erro"} @app.post("/api/webhooks/asaas") async def webhook_do_asaas(payload: dict, db: Session = Depends(get_db)): try: evento = payload.get("event") if evento in ["PAYMENT_RECEIVED", "PAYMENT_CONFIRMED"]: pagamento = payload.get("payment", {}) descricao = pagamento.get("description", "") if "#" in descricao: pedido_id_str = descricao.split("#")[1].split(" ")[0] pedido = db.query(PedidoModel).filter(PedidoModel.id == int(pedido_id_str)).first() if pedido and str(pedido.status).split('.')[-1].upper() != "RECEBIDO": pedido.status = "RECEBIDO" db.commit() return {"status": "ok"} except Exception as e: print(f"❌ Erro Webhook Asaas: {e}") return {"status": "erro"} # ==================================# 11. CHECKOUT & VENDAS (ONLINE E PDV) # ========================================= @app.post("/api/pedidos/online") definitivamente receber_pedido_site(pedido_web: CheckoutPedido, forma_pagamento: str = Consulta("entrega"), db: Sessão = Depends(get_db)): try: config = db.query(ConfiguraçãoLojaModel).first() cliente = db.query(ClienteModel).filter(ClienteModel.telefone == pedido_web.telefone_cliente).first() se não cliente: cliente = ClienteModel( nome=pedido_web.nome_cliente, telefone=pedido_web.telefone_cliente ) db.add(cliente) db.commit() db.refresh(cliente) itens_carrinho = [{"produto_id": i.produto_id, "quantidade": i.quantidade, "observacao": i.observacao} for i in pedido_web.itens] if getattr(pedido_web, 'endereco_cliente', Nenhum) e len(itens_carrinho) > 0: obs_atual = itens_carrinho[0].get("observacao", "") ou "" itens_carrinho[0]["observacao"] = f"Endereço: {pedido_web.endereco_cliente} | {obs_atual}" novo_pedido = registrador_venda_pdv( db=db, tipo=TipoPedido.DELIVERY, itens_carrinho=itens_carrinho, cliente_id=cliente.id ) novo_pedido_real = db.query(PedidoModel).filter(PedidoModel.id == novo_pedido.id).first() if novo_pedido_real: novo_pedido_real.senha_diaria = gerar_senha_diaria(db) novo_pedido_real.origem = "SITE (Online)" db.commit()
